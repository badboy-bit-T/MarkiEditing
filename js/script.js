...
$('#apply-marki').on('click', function () {
    ...
    // 🔹 cek checkbox cap-air
    const isCapAir = $('#toggle-cap-air').is(':checked');

    // hapus overlay lama jika ada
    currentWatermark.siblings('.cap-air').remove();

    if (isCapAir) {
        // tambahkan img overlay cap-air langsung di wrapper
        ...
        capAirImg.css({
            ...
            width: '100%',
            height: '100%',
            zIndex: 6,   // di atas gambar asli, di bawah tombol download
            pointerEvents: 'none'
        });
